...
刷新 / 切会话都在),并隐藏右栏的「分栏」「进全屏」两枚按钮 —— 用户要的全屏落在**详情浮窗**上,见下。
...
- **详情悬浮面板**:视口 1/2 宽、满高、紧贴右栏左缘,**同一时间只有一个**;切会话 / 切工作区 / 右栏折叠 /
  ...
  **文件名本身可点**(点一下复制文件名,只有染色反馈),而且**标题不再被提前截断**(长文件名显示全)。
  ...
  面板头部**只留「关闭」**(官方那枚会把详情变回右栏页签的「送回侧栏」已藏掉,且只藏本插件自己的浮窗)
  外加本插件加的**「全屏」**:**点一下详情面板铺满整个视口**,再点回到 1/2 宽贴右栏左缘那份几何
  (这枚只在浮窗里出现, 页签条上没有 —— 右栏顶部那两枚 chrome 按钮里没有它)。
...
- **刷新**:agent turn 结束时自动重取变更列表与历史首页(页签不可见时挂起、可见时补刷,且**不 fetch**);
  ...
  详情浮窗里 **`Alt`+滚轮 = 横向滚动**(长代码行 / 宽 diff 不必再去够底部那条横向滚动条)。
- **worktree 切换器**:仓库里用了 `git worktree`(比如 `.worktrees/<名字>`)时,git 页签头部多一颗按钮 ——
  点它选主仓或任意一份工作树,**分支名 / 变更列表 / 提交历史 / diff 全跟着那份走**(只改"看哪份",不动 checkout);
  没有别的 worktree 时这颗按钮不出现。
