...
- **行操作(悬停出现, 目录/文件行一致)**:`⧉` 复制绝对路径(成功后变绿 `✓`, 1.2s 还原)、文件夹图标在系统资源管理器中打开(目录开自身 / 文件开所在目录)、`⋮` 更多——`+ 新建`(仅目录行)/ `✎ 重命名` / `✕ 删除` 收进固定定位下拉(点面板外 / Esc / 滚动 / 缩放收起)。文件内容与 diff 悬浮面板头部新增「复制」按钮, 面板内 **Ctrl/⌘+A 只选中本面板正文**(不再选整页)。
