...
function build() {
    ...
    const onDisk = readdirSync(jsDir)
        .filter((f) => f.endsWith('.js'))
        .sort();
    const untracked = onDisk.filter((f) => !JS_FILES.includes(f));
    if (untracked.length > 0)
        throw new Error(`src/ 下有未加入 JS_FILES 的 JS 文件: ${untracked.join(', ')}`);
...
if (checkOnly) {
    ...
    if (existing !== out) {
        console.error('--check: lib/client.js 与 src/ 不一致');
        process.exit(1);
    }
    ...
    console.log(
        `已生成 lib/client.js（${out.length} 字节：CSS ${readdirSync(join(root, 'src', 'css')).filter((f) => f.endsWith('.css')).length} 个文件 + JS ${JS_FILES.length} 个文件）`,
    );
}
