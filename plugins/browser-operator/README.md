...
没有可选目标的那一问整个不发** —— `CLICK` / `TYPE_TEXT` / `SELECT` 各对应一问,逐字候选为空时
「填哪段文本」那一问也一样。原因有两条:空 `criteria` 的 `choice` 会被 Jev 拒收(400),而且没有
可选项时问了也没得选 —— 模型于是**没有机会**挑一个不存在的目标。`operation` / `goal_met` /
`stuck` 永远都在:页面上一个可操作元素都没有时,模型照样能选 `DONE` / `BLOCKED` / 滚动 / 等待。
