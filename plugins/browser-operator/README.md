...
**问题集按页面裁剪。** 一次请求里同时问完 `operation` 与「选哪个目标」的各问,但**当前页面
没有可选目标的那一问整个不发**,而且**没有合法目标的操作也不再出现在 `operation` 的选项里** ——
`CLICK` / `TYPE_TEXT` / `SELECT` 各对应一问,逐字候选为空时「填哪段文本」那一问也一样。原因有两条:
空 `criteria` 的 `choice` 会被 Jev 拒收(400),而且没有可选项时问了也没得选 —— 模型于是**没有机会**
挑一个不存在的目标;把一个本页根本执行不了的操作留在选项里,等于请它挑一个必然失败的答案。
`goal_met` / `stuck` 永远都在;`operation` 也永远非空 —— `SCROLL_UP` / `SCROLL_DOWN` / `WAIT` /
`DONE` / `BLOCKED` 五个都不需要目标,页面上一个可操作元素都没有时模型照样能选它们。

**它不做文本生成。** `TYPE_TEXT` 的文本必须**由 Jev 从逐字候选里挑**一个:候选由代码枚举
(goal 的片段 + **一个**页面字段标签 / 占位符切出的片段),选中哪一份就原样输入哪一份。没有可用片段时
它会以 `status: 'text_unavailable'` 停下,这时改用 `browser_fill`。

**要填确切文字,请传 `text`。** 参数给 `text: '…'` 时**只用它一个候选** —— 「填什么」由调用方定,
不再从 goal 里切片段。**中文目标句往往没有分隔符**,切词只能切出**整句指令**:实测
`goal: "在搜索框输入哥德尔不完备定理并搜索"` 时,候选里唯一 goal 片段就是这一整句,于是它被原样填进了
搜索框。凡是要往输入框填具体文字(搜索词、名称、地址…)的目标,都应当传 `text`。

**字段标签那半截候选只来自一个字段。** 候选必须在请求发出**之前**就写进 Jev 的选项里,而目标字段是
Jev 在**同一个请求里**才选的,没法等它 —— 所以代码取元素表里**第一个**能切出片段的可填字段的标签
(`lib/loop.js` 的 `labelSource`),它**不保证**就是最后被选中的那个目标。只有在「第一个可填字段正好
就是目标」时,候选才覆盖了目标字段的标签。goal 里能切出片段时,这只是少列几项;goal 里切不出可用片段时
(goal 为空,或 goal 里的词都不到 3 个字符),候选就只剩**那一个**字段的标签,页面上其它字段的标签不会
被问到 —— 于是 `text_unavailable` 会比「按目标字段标签切」更容易出现,这时改用 `browser_fill`。
...
**同一个操作被反复重复而页面毫无变化 → `status: 'no_progress'`。** 判据是「上一步执行过、这一份
观测与它决策时那份完全相同、模型又选了**同一个操作与同一个目标**」,连着 3 次就停。它和 Jev 自报的
`stuck` 是**两回事**,所以单列一个状态。为什么需要它:落点页上同一个下标可能解析到**另一个**元素
(比如页内锚点 `#…`),点它不改元素数 / URL / 正文长度,`freshness` 因此完全看不见 —— 实测有回路
在这上面连点 11 次直到步数上限。**只看页面的变化不够**(往输入框打字也不改那三样),所以判据里
同时要求操作与目标都相同。

**返回里带 token 用量。** `usage = { inputTokens, outputTokens, calls }` 统计**整次回路**的 Jev
开销,**含校验失败后重新观察的那几轮** —— 重试同样要付钱,`calls` 就是一共发了几次请求。要判断
「这一步贵不贵、贵在重试还是贵在页面太大」看它。

**动画页面先重试,持续在动才中止。** 陈旧性复检在**每一个**动作前都会跑,一旦对不上就
**重新观察、重试最多 2 次**,额度用尽才终止整次运行(此时 `status: 'error'`)。这是有意的安全
取舍(宁可停,也不拿过期的元素索引去操作);重试是为了吸收**良性**的页面变化 —— 走动的时钟、
改变可见文本长度的滚动条、新冒出来的元素都算,它们通常一轮重新观察后就稳了。**持续**在变的
页面仍会中止;那种页面改用单步工具。
...
state 与 questions 的大小,而这两份**每一步都整份重发** —— 12 步就是 12 次。上限页面离线复算
(200 元素 / 4000 字正文 / 12 步台账):单次请求体约 30 KB,其中元素表被 state 与各目标问的
`criteria` **各发一遍**,合计占 77%。

**字节能离线算准,token 不能。** 同样体量的载荷,英文页与 CJK 页的实际 input 相差三成以上:
真实运行观察到单次请求在 **7.6k–11.6k** 之间,中文页面最高(离线按 3.6 B/token 估出的 8.5k
落在中间)。所以调这两个值就是直接买 token,但**按返回里 `usage` 的实际读数调,不要按字节
估算调** —— `usage` 正是为这件事加进来的。
...
- **`browser_act` 的陈旧性复检每一步都跑**:对不上就重新观察重试(最多 2 次),额度用尽才以
  `status: 'error'` 收场。所以良性的页面变化 —— 走动的时钟、改变可见文本长度的滚动条、新出现的
  元素 —— 通常只多花一轮观察就过去了;**持续**在变的页面才会真的中止。宁可停,也不拿一个过期的
  元素索引去操作;页面本身一直在动时用单步工具。
