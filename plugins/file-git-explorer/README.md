...
- **上下两栏**:上栏是**变更列表 / 当前 diff**(默认占正文 **3/4**),下栏是**提交历史列表**或**聚焦提交** —— 两栏**各自滚动**,
  互不牵连(上栏滚到底不会把历史顶走)。中间那条 **5px 拖柄可以拖**(**悬停不变色**, 只把指针变成 `ns-resize`
  —— 与终端抽屉上缘那条同一口径),按正文高度换算、钳在 **40%~90%** ——
...
- **点文件看 diff 不会打断这里**: 打开的 diff 页签会浮起来, 回到 git 页签时**聚焦态 / 列表 / 滚动位置原样保留**
  (不会重新拉取、不会闪 —— 状态按会话缓存在模块级, 见实现事实 §10)。**手动刷新也不会把你从聚焦里踢出来。**
...
- **选中即复制, Alt+C 兜底; 条右侧有开关**: 终端里**拖选 / 双击选词 / 三击选行**一松手就进剪贴板(走官方
  ...
  **强制重写一遍**(自动那次可能被浏览器或权限挡了, 这时内容当然一模一样, 不能跳过)。两条路共用同一条
  ...
  条右端 `■` 左边那枚**开关**(`role="switch"` + `aria-checked`, 自己画的轨道 + 滑块)关掉的是**自动**那条 ——
...
- **页签样式**: 活动态页签底色取**终端表面色**(`--dsw-alias-markdown-code-block`, 与终端体同色)加两角 `6px 6px 0 0`;
  **页签与 `■` 在"拖柄 + 标题条"这条带子里垂直居中**(默认 `flex-start` / 贴底时它们会挨着下面那条分隔线,
...
⚠ 这条拖柄**没有 hover 底色**(与终端抽屉上缘那条**同一口径**, 见 §13): 它同样是一整条 5px 通宽的横带,
...
- **刷新键是官方 SVG 图标**:`primitives.IconRefreshOutline14`(20+ 个官方图标与分支 / 箭头图标同在一张表里,
  ...
  **自己这一格**的居中与 busy 动画(`inline-flex` + `@keyframes fge-spin`), 不动 `.fge-btn` 的通用盒子 ——
  返回键 / `■` 还各有自己的排版。⚠ **不要再退回 `⟳` 字形**:它是文字, 画风随平台字体漂移。
...
- 滚动位置用 `scrollBefore` / `restoreScroll` 两个 **ref** 记:进聚焦时存下 `scrollTop`,返回时置一次
  ...
  (按一个陈旧的值把列表跳走 = 新引入的毛病)。所以它**不进** §10 那份按会话缓存 —— 它是"这一次挂载内"的事。
...
10. composer 下方有抽屉舌;点它**向上**展开终端(能跑 `vim` / 颜色 / 补全);抽屉**顶部两角圆角**、
    ...
    **标题条那条分隔线在页签下面也不断**(页签不许压线), **页签与 `■` 在"拖柄 + 标题条"这条带子里垂直居中**
...
11. 全程 DevTools 控制台**零 pageerror**、零插件 `console.error`。
