...
- **Alt + 滚轮 = 横向滚动**(用户口径: "代替横向滚动条"): 面板只有半屏宽, 长行(代码 / diff / 宽表格)
  要看右边那段本来得去够面板**最下面**那条横向滚动条 —— 现在指针停在面板里按住 `Alt` 滚滚轮就行。
  ⚠ 横向**到边**、或这个浮窗里压根没有可横滚的东西时, 那一下**照旧是普通竖滚**(不会变成"滚轮失灵");
  段落 / 代码块内部自己那层可横滚的盒子优先(就近滚那一层)。
  ⚠ 只认**本插件自己的详情浮窗**。原理与限定见实现事实 §15。
...
「使用」里只留了一张表;这里放**为什么这么设** —— 四个 Alt 键 + 一条 `Alt`+滚轮手势, 各一条全局监听, 分流规则是统一的:
...
**`Alt` + 滚轮 = 详情浮窗横向滚动**(唯一一条鼠标手势, 与上面四个键同一族: 都挂全局监听、都先看清"这一下该归谁"):

- **为什么要有**: 详情浮窗只有半屏宽, 长行(代码 / diff / 宽表格)要看右边那段就得去够**面板最下面**那条
  横向滚动条 —— 指针离它太远。用 `Alt+滚轮` 把竖滚的量"倒"到横轴上。
- **挂 `window` 捕获 + `{ passive: false }`**: 被动监听里的 `preventDefault()` 会被忽略(那样横滚的同时
  竖滚也会发生)。⚠ 这条是**必踩项**: 少写 `passive: false` 时行为看着"只是有点怪", 很难联想到原因。
- **只认本插件自己的详情浮窗**: 判据同「送回侧栏」那条 —— 浮窗标题里有我们的文件名芯片
  (官方浮动宿主是所有页签共用的, 不去改别人的浮窗行为)。探针里专门放了一个"别人的浮窗"和一个页签条里的
  滚动容器, 都验过**纹丝不动**。
- **滚不动的那一下不吃**: 横向已经到边、或这个浮窗里没有可横滚的盒子时**直接放行** —— 于是
  `Alt+滚轮` 在没有横向余量时还是普通竖滚, 不会变成"滚轮失灵"。⚠ 判"滚不动"必须在 `preventDefault()`
  **之前**(探针里用 `defaultPrevented` 实测了这条)。
- **就近滚那一层**: 从事件目标往上找第一个**真有横向溢出且 `overflow-x` 是 auto/scroll** 的盒子 ——
  段落 / 代码块自己那层可横滚的容器优先, 都没有才落到浮窗体(官方那条 `overflow:auto`)。
- **只认 `Alt`**: 不带 Alt 的、带 `Ctrl`/`Meta` 的(浏览器缩放 / 系统手势)一概不碰。
- 探针(真浏览器 + 真滚轮输入, 8 项)实测: 我们那格 0 → 240 且**被吃下**(页面没竖滚) → 能一路滚到
...
12. **`Alt`+滚轮横向滚动**(详情浮窗里): 找一段横向超出的内容(长代码行 / 宽 diff), 指针停在浮窗里按住
