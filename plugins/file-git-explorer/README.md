...
- **3) 放开浮窗里那个页签的宽度上限**(`[data-dockkit-tab][class*="_floatTitle_"]{max-width:none!important}`):
  "文件名经常显示不全"(用户口径)的**真因在页签上, 不在标题上** —— 官方给页签钉的是
  `min-width:80px; max-width:170px`, 而**浮窗头部那个标题就是同一个页签元素**(只是多挂了一个
  `_floatTitle_` 变体), 于是半屏宽的浮窗里文件名可用宽度也只有 170px。
...
overflow:hidden` + 裁切时加 `mask-image` 渐隐)—— **第一版改它是空操作**, 这条踩过。
  ...
  真要改名了这条会静默失效, 所以护栏里有一条断言盯着它。
  真机量过(探针里**逐字抄来**官方那两条规则搭场景 + 注入本插件 CSS): 浮窗那格标题 170px(被截)→ 620px(不再被截),
...
> `.fge-chip-label` 撤掉自己的 `22em` + 文件名走 `DocName` 点击复制(复制的是文件名本身、
