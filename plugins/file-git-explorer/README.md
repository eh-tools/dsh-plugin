...
- **标题不再被提前截断**: 官方那层 `[data-dockkit-tab-title]` 把宽度卡得很死, 在**半屏宽的浮窗头部**
  文件名也常被截成 "…"。插件放开这个上限(见实现事实 §11 第 3 条), 真的没地方时才由官方那层 ellipsis 收尾。
...
- **3) 放开详情标题的宽度上限**(`[data-dockkit-tab-title]{max-width:none!important;min-width:0;flex:1 1 auto}`):
  官方那层把标题卡得很死, 于是在**半屏宽的浮窗头部**文件名也经常被截成 "…"(用户口径"文件名经常显示不全")。
  只放开**标题内部的宽度分配**, 不动页签本身的宽度 —— 页签条里还有别的格子时不会被挤走;
  真的没地方时仍由官方那层的 `ellipsis` 收尾。真机量过(探针里造一条同形状的假官方约束 + 注入本插件 CSS):
  标题 128px(8em, 被截) → 578px(clipped=false);容器真的很窄时仍然正常 ellipsis 且不溢出。
...
> 「详情标题放开 `[data-dockkit-tab-title]` 的宽度上限 + 文件名走 `DocName` 点击复制(复制的是文件名本身、
> **不吃掉点击**、反馈只染色)」,
...
8. 悬浮面板头部:文件名前有文件类型图标,后随一枚**复制图标**;点它 → 图标变「已复制」,剪贴板是磁盘原文
   ...
   **文件名要完整显示**(长文件名不再被截成 "…";真到没地方时才是省略号),**点文件名本身 → 剪贴板拿到这个文件名**
