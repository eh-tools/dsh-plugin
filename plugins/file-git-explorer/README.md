...
- 顶部: 当前分支(前有竖着 git 分支 SVG 图标; 实时读 `git branch --show-current`), 点击从**面板左侧**弹出**所有分支下拉**(本地 / 远程分组, 纯展示清单: 行不可点、无 hover 反色, 仅以「当前 / 查看中」标记状态, 不支持切换)。下拉与 diff / 提交历史悬浮栏**互斥**(开一关一, 两者同占面板左侧留白带, 避免互相遮挡)。
...
- 列表每页 50 条, 滚动到底自动追加(`--skip` 分页); 条目 = subject + 作者 · 相对时间 · 短 hash; **条目间有分割线**, 详情里文件行之间与展开的 diff 块底部同样有分割线。
