...
- **键盘切页签**:`Alt+J` 选**左边**一格、`Alt+L` 选**右边**一格(mac 上是 Option);**到边就什么都不做**
  (用户口径: 不环绕 —— 在最左还按 `Alt+J` 不会绕到最右, 免得丢掉方位感)。所以那两格之间就是来回切。
  算的是**页签条上视觉相邻**的那一格(不是写死这两格), 页签条里还有别的插件的格子时同样成立。
  焦点在**终端里**时让给终端;右栏**收起**时不切(切了也看不见);**输入框里照常可用**
  (`Alt+J` / `Alt+L` 不输入任何字符, 而在 composer 里打字时想切页签恰恰是最常见的时机 ——
  已确认官方没有任何 `Alt+J` / `Alt+L` 绑定:官方那几个 `altKey: "any"` 全在 ArrowUp / ArrowDown / Enter 上)。
- **键盘刷新 git**:`Alt+Ctrl+R` = 页签上那枚 `⟳`(同一条 `manualRefresh`: 先 sync/fetch, 再 info → status → 历史首页)。
  一个动作只有一个入口 —— 快捷键与按钮指向同一个函数, 不是另写一套刷新逻辑。
  **为什么带 Ctrl 而不是光 `Alt+R`**: 光 `Alt+R` 在终端里是 readline 的 revert-line, 而**展开抽屉后焦点就在终端里**
  (见「展开就自动聚焦终端」)—— 那时这一按该归终端, 用户看到的就是"按了没反应"(实测就是这么翻的车)。
  加 `Ctrl` 之后终端侧没有对应绑定, 于是它**能从任何焦点触发**(包括终端里), 也不再需要那道门。
  ⚠ 因此它认下就 `preventDefault` + `stopPropagation`: 不落到 xterm 变成发给 PTY 的字节, 也不让浏览器/官方再拿它做别的;
  ⚠ 挂在 **window 的捕获阶段**(不是 document): 官方快捷键也在捕获, window 更靠前, 万一官方先把它当别的用了我们仍收得到;
  ⚠ 输入框里照常可用;⚠ 右栏收起 / 当前那格不是 Git 时页签体没挂载 → **切到 Git 页签 + 记一笔待刷**,
  它挂载时立刻刷一次(否则这条键只在"正看着 git"时才灵)。
  已确认官方没有 `Alt+R` / `Alt+Ctrl+R` 绑定, 且浏览器不会先吃掉这一按(真机验过: 页面 keydown 收得到 `altKey + ctrlKey + KeyR`)。
...
- **展开就自动聚焦终端**(用户口径: 省掉"再用鼠标点一下才能打字")。做法是 `term.open(...)` **之后**调一次
  `term.focus()` —— xterm 的 focus 是打到它自己那个隐藏 textarea 上的, 元素没挂上去就等于没调。
  ⚠ **只在挂载那一次做**(终端 effect 的依赖是 `[root, visible]`, 不是每次渲染): 否则用户刚点去 composer
  打字, 一次无关重渲染就把焦点抢回来。真机验过: `focus()` 后 `document.activeElement` 落在 `.xterm` 里的
  textarea 上、敲键真进 `onData`; 不聚焦时敲键进不去。
  ⚠ **代价要说清**: 焦点一进来 `Esc` 就归终端了(与"焦点在终端里"那条分流一致), 所以**开抽屉后 Esc
  不再收起抽屉** —— 想收起点条空白处、页签上的 `×`, 或先把焦点点出去。
- **选中即复制, Alt+C 兜底; 条右侧有开关**: 终端里**拖选 / 双击选词 / 三击选行**一松手就进剪贴板(走官方
  `primitives.writeClipboard`), 不用再按任何键 —— 这是默认路径。**Alt+C** 是兜底: 内容与"自动那次"相同时也
  **强制重写一遍**(自动那次可能被浏览器或权限挡了, 这时内容当然一模一样, 不能跳过)。两条路共用同一条
  `copySelection(force, expandTail)`, 免得日后走岔。
  条右端终止键左边那枚**开关**(`role="switch"` + `aria-checked`, 自己画的轨道 + 滑块)关掉的是**自动**那条 ——
  Alt+C 照旧可用(它是兜底, 与开关无关), **选区收敛也照旧做**(那是选区本身的口径)。开关状态记在
  `fge-term-copy-v1`(整机一个偏好, 默认开); ⚠ 它长在"整条可点即收起"的标题条里, 所以它的 `onClick`
  第一件事就是 `stopPropagation`, 否则一按开关就把抽屉收了。
  ⚠ 开关的尺寸**全套取偶数**(盒子 18 与终止键一致 / 轨道 24×12 / 滑块 8 / 文字行盒 12): 条里内容行高是
  **奇数**(页签 21px), 控件自己若是 17 / 轨道 13 / 行盒 11.5, 居中就会落在 `.5px` 上 —— 相邻元素的
  **文字与几何各自吸到不同的半像素**, 看着就是"文字和开关垂直没对齐"。全取偶数之后, 真机量到:
...
- **不许把选区拖进"内容下方那片空白"**(用户报的 bug: 拖到底选中一大堆空行, 复制出一串换行)。
...
- **终止键**(官方 SVG `IconStopFill16`)终止该工作区终端整棵进程树;**页签上的 `×` / 点标题条空白处**
  只是收起抽屉, **不杀进程** ——
  抽屉关闭 / 切换会话 / 刷新页面都不影响它。
