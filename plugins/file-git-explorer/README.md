...
- **Git 页签**(kind `fge-git`, docked 常驻): 头部一颗**分支按钮**(当前分支 + 上游 `↑ahead ↓behind` 徽标),
  ...
  正文是**上下两栏** —— 上栏**变更列表 / 当前 diff**(默认 3/4)、下栏**提交历史**(分页),两栏各自滚动,
  中间拖柄可调占比;两份列表都**按目录归类**;点一条提交**就地展开**它的文件(带 ±行数,说明默认折两行、可展开),
  页签内没有「返回」;点任一文件 → diff 进**悬浮面板**。
...
右栏点「Git」。页签正文是**上下两栏**, 两栏**各自滚动**、各自吸顶, 没有视图栈、没有返回按钮:

```
头部:分支按钮   当前分支(带分支图标 + 箭头);点它弹出**分支小浮窗**(见下)+ 上游 ↑↓ + ⟳
── 上栏(默认占正文 3/4)──────────────────────────────────────────────
变更列表        工作区相对 HEAD 的全部变更(已暂存 + 未暂存 + 未跟踪), **按目录归类**, M/A/D/R/U/? 徽标
── 拖柄: 拖动调占比、双击回到 3/4 ────────────────────────────────────
── 下栏(默认占正文 1/4)─────────────────────────────────────────────
提交历史        按时间倒序; 节头显示正在查看哪个分支 + 每页 50 条
  └─ 点一条提交 就地展开: 提交说明 + 文件清单(±行数, **也按目录归类**); 再点收起
       └─ 点文件 diff 进悬浮面板
  └─ 加载更多…  每页 50 条
```

- **上下两栏**:上栏是**变更列表 / 当前 diff**(默认占正文 **3/4**),下栏是**提交历史** —— 两栏**各自滚动**,
  互不牵连(上栏滚到底不会把历史顶走)。中间那条 **5px 拖柄可以拖**,按正文高度换算、钳在 **40%~90%** ——
  也就是**下栏最多占 60%**(往上拉不会把变更列表挤没;正文高度 = 整页减 38px 页签头,所以这条约束对整页只会更严),
  下栏最少留 10%。**双击拖柄回到 3/4**。拖过的比例记在 `localStorage`(刷新 / 切会话都在),没拖过就是 3/4。
- **按目录归类**:两份列表都按路径折成**目录树**, 而不是每个文件都从头平铺一遍完整路径 ——
  ...
  目录在前、文件在后;完整路径仍在这两处的 `title`(悬停可见),请求 diff 用的也是完整路径。
- **提交说明折两行**:展开一条提交后,最上面那块说明默认**只显示两行**(右栏最窄 200px,一段带 body 的
  message 铺开会把下面的文件清单挤出视野);**放不下**才出现「展开 / 收起」,点开看全文;
  短于两行的说明没有这枚按钮。是否放得下是**量**出来的(`scrollHeight > clientHeight`),不按行数猜 ——
  一行很长的 subject 折行后同样给得出出口。**开关画在说明文字的上方**:展开之后它原地不动,
  不用再去拉滚动条找「收起」。展开态与「哪条提交是展开的」一起按会话缓存,点文件开 diff 再回来仍在。
...
- **变更列表**: 相对 `HEAD` 的全部变更, 按路径排序, 未跟踪沉底(上屏时按目录归类, 见上)。
...
拖柄是 5px 的空 div(`.fge-grip`, 上边一条 `border-l3` 当作两栏的分界线), move/up 挂 `document`
...
- 拖拽区间是 `GIT_SPLIT_MIN..GIT_SPLIT_MAX = 40..90`:上限是"下栏至少留一条", **下限是用户口径
  「往上拉别超过页面高度的 60%」**。正文 = 整页高度减 38px 页签头, 所以"下栏 ≤ 60% 正文"**一定**
  ≤ 60% 页面 —— 这条约束就落在这一个常量上, 不需要去量视口、也不受窗口尺寸影响。
  旧版本存下的 15 之类会被 `readGitSplit` 夹回 40。

**按目录归类**:`buildPathTree(items, pathOf)` 折树 → `compactPathTree` 压单链目录 → `treeRows`
上屏;变更列表与提交展开的文件清单共用这一条链, 只是深度起点不同(0 / 1, 后者对应原来的 `.fge-indent`)。
...
- ⚠ 提交里展开的文件行**不能用循环变量做闭包**: `commit` 是 `var`, 上一轮的行会拿到最后一条提交。
  工厂函数里先取 `var hash = commit.hash`, 再让 `onClick` 闭这个局部。
...
- ⚠ **开关(`.fge-msg-bar`)画在文字上方**: 用户口径是"展开后别让「收起」跑到最底下、还得拉滚动条"。
  放在文字下面时, 展开会把按钮顶到全文末尾 —— 位置随内容变。量的是 `.fge-msg-text` 自己,
...
- 展开态(`msgOpen`, 键 = commit hash)与 `expanded` 同款**按会话缓存**在 `gitViews` 里(见 §10),
  点文件开 diff 再回来仍在;换工作区时与 `expanded` 一起清空。
- 点**说明文字本身**不切换 —— 那段文字可以拖选复制, 只有「展开 / 收起」按钮是开关。

> 由 `scripts/verify-client-bundles.mjs` 的离线守住: 「`.fge-body` 是竖排 flex + 两栏各滚各的 + 有 `ns-resize` 拖柄 +
> 上栏默认 75 + 下限 40(下栏 ≤ 60%)」, 「两份列表都真的走了 `treeRows`」, 「说明折叠态 `-webkit-line-clamp:2` +
> 有 `.fge-msg-toggle` + 靠量而非按行数猜 + 开关渲染在文字之前」, 「默认页签恰好铺「文件」+「Git」这一对」,
> 以及 `exports.__pathTree` 上的纯函数断言(同一目录只出一行 / 文件名是 basename / 单链压成一行 / 目录在前文件在后)。
> `__pathTree` 是**只为离线校验**递出去的出口(浏览器 bundle 不能 require 本包自己的模块), 不是插件契约。
> 真浏览器验收见验收清单第 1、2、4 条。
...
2. 正文是**上下两栏**: 上栏**变更列表**(默认占正文 3/4 高)、下栏**提交历史**,**两栏各自滚动**(滚一栏不动另一栏);
   ...
   **往上拉到头时下栏不会超过页面高度的 60%**(变更列表始终留着);
   两份列表都**按目录归类** —— 同一目录只有一行目录名(带文件夹图标)、文件名缩进列在它下面,
...
4. 点一条提交 → **就地展开**说明 + 文件清单(±行数, 同样按目录归类; merge 提交只出说明);再点收起;「加载更多…」能翻页。
...
「git 页签分栏」「目录归类」「composer 座」「抽屉舌」「终端抽屉」「终端标题条」「工作区终端」「回放缓冲」「工作区」的定义见仓库根 `CONTEXT.md`。
