...
- **不消失**: 执行后命令文本保留在输入框(不清空); ↑/↓ 在历史间导航(相邻去重, 上限 100 条),
  ...
  **输入框右缘内常驻运行状态点**(绿色 = 运行中/启动中, 红色 = 已停止), pointer-events 穿透不挡输入; 修改/清空命令即作废上一任务(状态点回到红色); 刷新/切工作区认领到终态任务时命令文本回填输入框。
...
- **尾部输出窗**: 输出窗**自带头部**(「输出」标签 + 收起按钮), 运行中每 ~1s 拉一次增量(offset 制非消费式读), 显示尾部各流 ~16K 字符, 自动滚底; stderr 以 `[stderr]` 段落区分; 启动失败等错误信息也写入窗内。
