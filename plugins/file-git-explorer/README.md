...
- **2) 隐藏「分栏」`[data-dockkit-split-button]` + 详情浮窗的「送回侧栏」, 保留「进全屏」`[data-sidebar-right-mode="fullscreen"]`**:
  ⚠ 这两个按钮曾**一起**被藏起来(前一个提交的取舍), 后来按用户要求把「进全屏」放了回来 —— 藏它是错的,
  因为**「进全屏」与「退出全屏」是同一个按钮**(官方 `PanelChrome`: `next = fullscreen ? "push" : "fullscreen"`),
  非全屏时它才带 `"fullscreen"`, 所以藏掉 = **非全屏时永远进不去**。
  ⚠ 放回来**不会**跟本插件那两条宽度规则打架 —— 官方面板进全屏时自己换成
  `data-sidebar-right-panel="fullscreen"` + `position:fixed;inset:0`(`width:100%`),
  全屏时它铺满整个视口(左栏也一起被盖住):
  ① 限宽那条写的是 `[data-sidebar-right-panel="push"]`, 全屏**不适用**(探针实测 `max-width: none`);
  ② 面板是 fixed 覆盖整个过程, grid 那一轨多宽都看不见(官方此时也不渲染右栏拖柄:
  `layoutInfo.rightbarShown && !layoutInfo.rightbarFullscreen && …`)。
...
> 「右栏 chrome 只藏「分栏」(`[data-dockkit-split-button]`), 而「进全屏」
> (`[data-sidebar-right-mode="fullscreen"]`)必须放回来 —— 它和「退出全屏」是同一个按钮」,
...
0. **右栏外观与拖动**:默认宽度 = 15vw(1920 窗口下约 288px);**打开右栏是把中栏挤窄、不是浮在它上面** ——
   ...
   右栏顶部是「收起」+「进全屏」+「+」,**只有「分栏」不在**;
   **点「进全屏」→ 右栏铺满整个视口**(左栏也一起被盖住), 再点一次(这时它的图标/提示是"退出全屏")回到原状,
   回来时仍是刚才那一格页签与宽度;
