...
- 文档:根 README 把 fge 移入「已归档」并删掉安装命令与使用说明小节;**删掉随之过期的「效果展示」全览图**
  (`png/home.png` 里还画着右栏 Git 页签与底部终端抽屉),以后要展示再重拍;AGENTS.md 的静态双半包参照换成在役
  插件、补上 `plugins/obsolete/` 归档口径;根 `CONTEXT.md` 的 § file-git-explorer 词表整节搬进归档目录
  (`plugins/obsolete/file-git-explorer/CONTEXT.md`),`docs/adr/0002`–`0006` 标注「插件已归档,仅作历史记录」
