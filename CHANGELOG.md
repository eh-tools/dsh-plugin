...
- `file-git-explorer`:shell 行动作与状态改版——「执行」「停止」改为 **▶ / ■** 图标钮;运行状态改为**输入框右缘内的常驻彩色圆点**(绿色 = 运行中/启动中, 红色 = 已停止, 默认红), pointer-events 穿透不挡文本输入;有效信息(退出码/错误/输出)全部在输出窗里, 输出窗**自带头部**(「输出」标签 + 收起按钮);修改/清空命令即作废上一任务(状态点回到红色, 输出保留可查), 刷新/切工作区认领到终态任务时命令文本回填输入框
