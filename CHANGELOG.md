...
- `file-git-explorer`:shell 行状态收敛为**始终显示的生命周期符号**——输入框与 ✓ 之间常驻 `○ 启动中 / ● 运行中 / ■ 已停止`(默认 ■,不随命令修改隐藏),点击符号开合输出窗;有效信息(退出码/错误/输出)全部进输出窗(启动失败错误也写入窗内);修改/清空命令即作废上一任务(符号回到 ■,输出保留可查),刷新/切工作区认领到终态任务时命令文本回填输入框;状态符号只占 22px, 输入框占位符不再被压缩截断
