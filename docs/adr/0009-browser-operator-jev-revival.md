...
- **终止状态从 8 种变成 9 种:新增 `no_progress`。** 上面决策点 6 把 `status` 写成闭合的 8 种,
  ...
  正文长度,`freshness` 因此对所有「空操作」完全盲,回路只能一路点到步数上限。
  **上线后同一任务、同一起点实测:步数 12 → 5、calls 12 → 5、input 87,205 → 36,199(-58.5%)、
  elapsed 8,873 ms → 3,656 ms**,收场状态 `max_steps` → `no_progress`;那一次的 steps 里
  **零** `执行前校验失败`,说明停下来靠的是新判据本身,不是陈旧性重试的副作用。同时用第 5 轮那个
  **成功**的任务做了回归:仍是 `done`,没有过度开火。**与 Jev 自报的 `stuck` 分开单列**,因为两者
  来源不同、混用会让台账读不准。判据刻意不只看 `freshness`:往输入框打字同样不改那三样,所以必须
  同时要求 `(operation, targetIndex)` 与上一步相同,连着 3 次才停(与 `LOW_CONFIDENCE_RUN` 同取 3)。
- **`browser_act` 新增可选参数 `text`(决策点 6 的第 6 条补充)。** 决策点 6 定的是「文本必须是 goal 里
  的逐字片段,由代码枚举候选、Jev 用 `choice` 挑一个」——**这条对没有分隔符的中文目标句不成立**:
  `textCandidates` 按空白与标点切词,「在搜索框输入哥德尔不完备定理并搜索」切出来只有一项,就是整句
  指令本身。实测它被原样填进了维基百科搜索框,而 `status` 是 `done`、`goalMet=0.53`——**看起来成功**。
  修法不是放宽"不生成文本"的立场,而是承认**文本的来源可以是调用方**:给 `text` 时只用它一个候选,
  回路不再从 goal 切片。这不引入第二个模型(调用方本来就在场),也不改 `choice` 机制的形状,只是把
  「填什么」这一步从候选枚举里挪回调用方。中文等无分隔符语言下,这是唯一可靠的来源。
  **上线后同一 goal 实测:3 步 / 19,752 in,直接落到条目页 `wiki/哥德尔不完备定理`,
  页面正文里也搜不到「在搜索框输入」** —— 而此前不带 `text` 时是 5–7 步 / 32k–43k in、落到
  `search=整句指令` 的结果页。修对了输入串之后,步数与 token 一并降下来。
