...
**复制内容(copy source)**:
文档页签 **⋯ 菜单**末尾的一项, 把该文件**磁盘上的原始文本**(如 markdown 源码)写进剪贴板。复制源是磁盘原件(经官方 `remote.workspaceFiles.readAll`), **不是**预览已渲染的缓冲, 因此与「预览渲染到哪了」无关。只加菜单项: 不替换任何正文组件、不碰 `documentPreviews` 注册表, 卸载即还原。
_Avoid_: 复制渲染结果、导出、下载

**渲染型文件(rendered file)**:
没有「原始文本」可复制的那一类预览 —— `html` / `xhtml` / `pdf` 由浏览器渲染, 图片是二进制。**复制内容**对它们不出现。判据是扩展名黑名单, 不做内容嗅探。
_Avoid_: 二进制文件(过宽: 二进制里也有可复制原文的情形)、富文本(那是渲染结果的说法)
